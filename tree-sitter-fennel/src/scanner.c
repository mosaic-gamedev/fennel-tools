...
typedef enum TokenType {
	// Reader Macros — order must match externals[] in grammar.js
	TK_HASHFN,
	TK_QUOTE,
	TK_QUASI_QUOTE,
	TK_UNQUOTE,
	TK_UNQUOTE_SPLICE,  // two-character token: ,@
...
// Comma-based tokens (TK_UNQUOTE, TK_UNQUOTE_SPLICE) are 0 here because
// they are handled specially in scan_reader_macro — commas can also be
// whitespace separators in Fennel, and ,@ is a two-character sequence.
static const uchar READER_MACRO_CHARS[TK_READER_MACRO_COUNT] = {
	[TK_HASHFN]         = '#',
	[TK_QUOTE]          = '\'',
	[TK_QUASI_QUOTE]    = '`',
	[TK_UNQUOTE]        = 0,  // handled specially
	[TK_UNQUOTE_SPLICE] = 0,  // handled specially
};
...
static ScanResult scan_reader_macro(TSLexer *lexer, const bool skipped_hashfn) {
	...
	if (skipped_hashfn) {
		...
		// Comma is special: ,@ is unquote_splice; bare , before a non-whitespace
		// non-bracket character is unquote; otherwise comma is a whitespace separator.
		lexer->advance(lexer, false);
		if (lexer->lookahead == '@') {
			lexer->advance(lexer, false);
			if (iswspace(lexer->lookahead) || is_close_bracket(lexer->lookahead) || lexer->eof(lexer)) {
				return SCAN_FAILURE;
			}
			lexer->mark_end(lexer);
			lexer->result_symbol = TK_UNQUOTE_SPLICE;
			return SCAN_SUCCESS;
		}
		// Bare comma: only an unquote when followed by a valid sexp-starting character
		if (iswspace(lexer->lookahead) || is_close_bracket(lexer->lookahead) || lexer->eof(lexer)) {
			return SCAN_FAILURE;
		}
		lexer->mark_end(lexer);
		lexer->result_symbol = TK_UNQUOTE;
		return SCAN_SUCCESS;
	} else {
		for (size_t tk = 0; tk < TK_READER_MACRO_COUNT; tk++) {
			const uchar ch = READER_MACRO_CHARS[tk];
			if (ch != 0 && lexer->lookahead == ch) {
				reader_macro_matched = true;
				reader_macro = tk;
				break;
			}
